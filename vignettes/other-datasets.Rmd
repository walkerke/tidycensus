...
__Please note: these features are currently available in the development version of tidycensus.  You can install with `devtools::install_github("walkerke/tidycensus")`.__
...
ggplot(compare, aes(x = AGEGROUP, y = value, fill = SEX)) + 
  ...
  scale_x_discrete(labels = function(x) gsub("Age ", "", x)) + 
...
My next project prior to the 1.0 release of tidycensus is to incorporate access to the [American Community Survey Migration Flows API](https://www.census.gov/data/developers/data-sets/acs-migration-flows.html).  Flows data will come integrated with simple feature geometry allowing for visualization of migration flows.  Stay tuned!